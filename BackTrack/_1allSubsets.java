...
public class _1allSubsets {
    ...
    // 第 4 个参数 result 是所有已经生成的子集。 



    // 不考虑将该数字添加到子集中时，不对子集进行任何操作，只需要调用递归函数 helper 处理数组 nums 中的下一个数字（下标增加 1）就可以。 
    ...
    private void helper(int[] nums, int index, LinkedList<Integer> subset, List<List<Integer>> res) {
        if (index == nums.length) {
            // 当 index 等于数组 nums 的长度时，表示数组中的所有数字都已经处理过，因此已经生成了一个子集，于是将子集 subset 添加到 result 中。
            // 需要注意的是，在 result 中添加的是 subset 的一个拷贝，而不是 subset 本身。
